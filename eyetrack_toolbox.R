...
#   # ------------------------------------------------------------------
#   fixations <- aggregate.fixations(sample)
...
#   } 
#   
#   labels <- c('trial','start','end','dur','event')
...
#   
#   # return(result)
#   return(list(summary=summary,fixations=fixations,saccades=saccades,blinks=blinks))
# } 
...
# 2. SUBJ: list of subjects to analyze
# 3. VFAC: analyze saccades obtained at VFAC value
# 4. max_t_dur: minimum duration in ms for the saccades to analyze
# 5. ECC: eccentricity values to analyze
# 6. amplitudefilt: minimum amplitude of the saccades to analyze
# 7. list.trials: a dataframe containing these columns: 
#     - 'label','trial','subID', varcond, 'ecc'
# 8. etr.db: whole EyeTracker database containing these columns:
#     - "RECORDING_SESSION_LABEL"
#     - "TRIAL_INDEX"
#     - "lxdeg"
#     - "lydeg"
...
# 9. ops.data.db: a behavioural dataframe (OpenSesame - ops) contanining:
#     - "image" <- this should be the same as labels
#     - "ecc" <- eccentricity values
#     - varcond <- condition variable to analyze
#     - "subID" <- subject ID
#     - "response" <- registered response
#     - "correct response" <- correct response
# 10. eye: write the eye to report -> 'left' or 'right'
# 11. varcod: name of the condition variable to analyze
#===============================================================================
analysis <- function(saccades.db, blinks.db, SUBJ, vfac, max_t_dur, ECC, 
                     amplitudefilt,list.trials, etr.db, ops.data.db, EYE, 
                     varcond){
  
  if(missing(varcond) | is.null(varcond) ){
    stop('varcond is missing. This analysis is designed given a variable condition')
  } 
  
  if(!any(names(list.trials) == 'ecc')){
    stop('Make sure your list has the eccentricity column: ecc')
  } 
  
  saccades.db <- saccades.db[which(saccades.db$subID %in% SUBJ),]
  blinks.db <- blinks.db[which(blinks.db$subID %in% SUBJ),]
  
  # saccades.db.vfac <- subset(saccades.db, VFAC == vfac & ecc == ECC & eye==EYE)
  
  saccades.db.vfac <- subset(saccades.db[saccades.db$ecc %in% ECC,] , 
                             VFAC == vfac & eye==EYE)
  
  # blinks.db.vfac <- subset(blinks.db, VFAC == vfac & ecc == ECC & eye==EYE)
  
  blinks.db.vfac <- subset(blinks.db[blinks.db$ecc %in% ECC,], 
                           VFAC == vfac & eye==EYE)
  
  # delete both above 40 ms and the amplitudefilt
  # saccades.below <- data.frame()
  # for (eccentr in ECC){
  #   below <- subset(saccades.db.vfac, t_dur <= max_t_dur & ecc==eccentr & 
  #                     amp <= amplitudefilt)
  #   saccades.below <- rbind(saccades.below,below)
  # }
  
  saccades.below <- subset(saccades.db.vfac[saccades.db.vfac$ecc %in% ECC,],  
                             t_dur <= max_t_dur & amp <= amplitudefilt)
  
  saccades.above <- anti_join(saccades.db.vfac, saccades.below)
  
  # Sanity check, this must be true
  nrow(saccades.db.vfac) == (nrow(saccades.above)+nrow(saccades.below))
  
  # first method
  above.trials<-unique(saccades.above[,c("label","trial","subID")])
  # length(which(unique(saccades.below$label) %in% unique(saccades.above$label)))
  # length(which(!unique(saccades.above$label) %in% unique(saccades.below$label)))
  
  # here we have to find the interaction trial-subject
  good.trials <- saccades.below[-which(interaction(saccades.below[,c("label","trial","subID")]) 
                                       %in% interaction(above.trials)),]
  ...
  trials <- etr.db[,c(xy,"RECORDING_SESSION_LABEL","TRIAL_INDEX","subID","ecc")]
  names(trials) <- c('x','y','label','trial','subID','ecc')
  
  trials <- trials[trials$subID %in% SUBJ & trials$ecc %in% ECC,]  
  
  list.trials.clean <- trials %>%
    group_split(trial,subID) %>%
    map_dfr(~trial.length.check(.x)) 
  
  list.trials <- list.trials[list.trials$subID %in% SUBJ & list.trials$ecc %in% ECC,] 
  
  list.trials.processed <- list.trials
  # also save the levels of the condition variable
  var <- unique(list.trials[,varcond])[order(unique(list.trials[,varcond]))]
  
  # delete variables that will not be used more  
  list.trials <- list.trials[,!(names(list.trials) %in% c(varcond,'ecc'))] 
  
  # now the difference will be the NA trials
  list.trials.nosignal <- list.trials[- (which(interaction(list.trials) 
             %in% interaction(list.trials.clean))),]
  # SANITY CHECK : 
  nrow(list.trials) == nrow(list.trials.nosignal) + nrow(list.trials.clean)
  
  # lets split lisstrials_clean (which are the trials with ETR data) into trials 
  #  with saccade detection and trials without saccade detection
  list.trials.saccade <- list.trials.clean[which(interaction(list.trials.clean) 
             %in% interaction(saccades.db.vfac[,c("label","trial","subID")])), ]
  list.trials.nosaccade <- list.trials.clean[-(which(interaction(list.trials.clean) 
             %in% interaction(saccades.db.vfac[,c("label","trial","subID")]))), ]
  # SANITY CHECK:
  nrow(list.trials.clean) == nrow(list.trials.saccade) + nrow(list.trials.nosaccade)
  
  # in the saccs are some that are good.trials (below 2 visual angles on 
  # amplitude and below 40 ms)
  list.trials.good.sacs <- list.trials.saccade[which(interaction(list.trials.saccade) 
             %in% interaction(good.trials[,c("label","trial","subID")])),]
  list.trials.bad.sacs <- list.trials.saccade[-which(interaction(list.trials.saccade) 
             %in% interaction(good.trials[,c("label","trial","subID")])),]
  
  # BIG SANITY CHECK....
  if(!nrow(list.trials) == nrow(list.trials.nosignal) + 
     nrow(list.trials.nosaccade) + 
     nrow(list.trials.good.sacs) + 
     nrow(list.trials.bad.sacs)){
    stop('The number of trials is not equal to the sum of trials with saccades, 
          without saccades and with high signal loss')
  }  else{
    sprintf('The number of trials is equal to the sum of trials with saccades 
    (n=%i below the threshold and n=%i beyond the threshold), 
    without saccades (n=%i) and with high signal loss (n=%i)',
            nrow(list.trials.good.sacs),nrow(list.trials.bad.sacs),
            nrow(list.trials.nosaccade),nrow(list.trials.nosignal))
  }  
  
  # now see blink percentage
  # get blinks present in trials with no saccade detection: list.trials.blinks  
  list.trials.blinks <- list.trials.nosaccade[which(interaction(list.trials.nosaccade)
              %in% interaction(blinks.db.vfac[,c("label","trial","subID")])),] 
  # add blinks present in trials with saccades below the threshold  
  list.trials.blinks <- rbind(list.trials.good.sacs[which(interaction(list.trials.good.sacs)
              %in% interaction(blinks.db.vfac[,c("label","trial","subID")])),] ,
              list.trials.blinks)
  
  # now get trials with without saccade detection and without blinks: list.trials.noblinks  
  list.trials.noblinks <- list.trials.nosaccade[-(which(interaction(list.trials.nosaccade)
              %in% interaction(blinks.db.vfac[,c("label","trial","subID")]))),] 
  
  # now get trials with saccades below the threshold without blinks: list.trials.good.sacs  
  list.trials.good.sacs <- list.trials.good.sacs[-(which(interaction(list.trials.good.sacs)
              %in% interaction(blinks.db.vfac[,c("label","trial","subID")]))),] 
  
  nrow(list.trials) == nrow(list.trials.good.sacs) + nrow(list.trials.bad.sacs) +
   + nrow(list.trials.nosignal) + nrow(list.trials.blinks) +
    nrow(list.trials.noblinks)
  
  list.trials.processed$nosacc <- NA
  list.trials.processed[which(interaction(list.trials) %in% 
                                interaction(list.trials.nosignal)),'nosacc'] <- 2
  list.trials.processed[which(interaction(list.trials) %in% 
                                interaction(list.trials.noblinks)),'nosacc'] <- 1
  list.trials.processed[which(interaction(list.trials) %in% 
                                interaction(list.trials.good.sacs)),'nosacc'] <- 1
  list.trials.processed[which(interaction(list.trials) %in% 
                                interaction(list.trials.blinks)),'nosacc'] <- 3
  list.trials.processed[which(interaction(list.trials) %in% 
                                interaction(list.trials.bad.sacs)),'nosacc'] <- 0
  
  total.trials <- data.frame()
  hcorrect.line <- data.frame()
  
  for (sbj in SUBJ){
    for(vr in var){
      for (eccentr in ECC){
        aux <- list.trials.processed[list.trials.processed$subID == sbj & 
                                       list.trials.processed[,varcond]  == vr & 
                                       list.trials.processed$ecc == eccentr,]
        sacc <- aux[aux$nosacc == 0,]
        nosacc <- aux[aux$nosacc == 1,]
        nullt <- aux[aux$nosacc == 2,]
        blinks <- aux[aux$nosacc == 3,] 
        
        aux.total.trials <- data.frame(matrix(ncol=7,nrow=1))
        aux.hcorrect.line <- data.frame(matrix(ncol=5,nrow=4))
        
        colnames(aux.total.trials) <- c('subID','nullper','saccper',
                                        'nosaccper','blinkper',varcond,'ecc')
        colnames(aux.hcorrect.line) <- c('subID','correctper',varcond,
                                         'ecc','variable')
        
        aux.total.trials[,'subID'] <- sbj
        aux.total.trials[,'saccper']  <- (nrow(sacc)/ nrow(aux))*100
        aux.total.trials[,'nosaccper'] <-(nrow(nosacc)/ nrow(aux))*100
        aux.total.trials[,'nullper'] <- (nrow(nullt)/ nrow(aux))*100
        aux.total.trials[,'blinkper'] <- (nrow(blinks)/ nrow(aux))*100
        aux.total.trials[,varcond] <- vr
        aux.total.trials[,'ecc'] <- eccentr
        
        # 1 and 3 are trials with no saccade adn with blinks respectively  
        aux_osesame <- ops.data.db[which(interaction(ops.data.db[,c('image','subID')]) 
            %in% interaction(aux[aux$nosacc %in% c(1,3),c('label','subID')])),] 
        aux.hcorrect.line[,'subID'] <- sbj
        aux.hcorrect.line[,'correctper'] <- c((sum(as.numeric((aux_osesame[,"response"] 
             == aux_osesame[,"correct response"])))/nrow(aux))*100,NA,NA,NA)
        # because stack is from 0 to 1 
        aux.hcorrect.line[,varcond] <- vr
        aux.hcorrect.line[,'ecc'] <- eccentr
        aux.hcorrect.line[,'variable'] <- c('nosaccper','saccper','nullper','blinkper') 
        
        total.trials<- rbind(total.trials, aux.total.trials)
        hcorrect.line <- rbind(hcorrect.line, aux.hcorrect.line)
      } 
    } 
  } 
  
  total.trials.melted <- melt(total.trials, id = c("subID",varcond,"ecc"))
  
  total.trials.melted$variable <- factor(total.trials.melted$variable, 
                                         levels = c("nullper","saccper",
                                                    "blinkper","nosaccper"))
  
  return(list(total.trials.melted=total.trials.melted,
